...
PLOT_LABELS = {
    ...
    "get_discussion_leader_bench": "Discussion Leader",
}
...
# Plot 50th and 95th percentile on one figure
def PlotMergedPercentilesNoBreak(baseline, alohomora, k9db):
    ...
    for percentile in PERCENTILES:
        ...
        k = [k9db[endpoint][percentile] for endpoint in ENDPOINTS_PLOT]
        ...
        label_baseline = "Baseline" if percentile == "50" else None
        label_alohomora = "Sesame" if percentile == "50" else None
        label_k9db = "Sesame+K9db" if percentile == "50" else None

        ax1.bar(X - 0.2, b, W, label=label_baseline,
                color=SYSTEM_COLORS['Baseline'], alpha=alpha)
        ax1.bar(X, a, W, label=label_alohomora,
                color=SYSTEM_COLORS['Sesame'], alpha=alpha)
        ax1.bar(X + 0.2, k, W, label=label_k9db,
                color=SYSTEM_COLORS['Sesame_K9db'], alpha=alpha)
    ...
    plt.savefig("k9db.pdf", format="pdf",
                bbox_inches="tight", pad_inches=0.01)
...
def ParseK9dbFiles(dir):
    data = dict()

    for endpoint in ENDPOINTS_LOAD:
        df = pd.read_json(dir + "/" + "k9db" +
                          endpoint + ".json")[0] / 1000000
...
if __name__ == "__main__":
    ...
    k9db = ParseK9dbFiles('benches')
    print(baseline, alohomora, k9db)
    ...
    PlotMergedPercentilesNoBreak(baseline, alohomora, k9db)
