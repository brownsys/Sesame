...
fn users_can_send_groupchats() {
    ...
    for s in 0..users.len() - 1 {
        ...
        let response = client
            .post(format!("/chat/testgroupchat/{sender}/send"))
        ...
        assert!(response_contains(
            &client,
            format!("/chat/testgroupchat/{sender}"),
            &message,
            ResponsePortion::Any
        ));
    }
...
fn users_can_recieve_groupchats() {
    ...
    for member in members {
        assert!(response_contains(
            &client,
            format!("/chat/testgroupchat/{member}"),
            &message,
            ResponsePortion::Any
        ));
    }
...
fn nonmembers_cant_see_groupchats() {
    ...
    let non_members = [
        "testgroupchat/julia",
        "testgroupchat/robert",
        "testgroupchat2/samantha",
        "testgroupchat2/steven",
        // ^^those not in either group
        "testgroupchat/charlie",
        "testgroupchat2/cher",
    ];
...
fn users_can_delete_their_own_chats() {
    ...
    assert!(response_contains(
        &client,
        "/chat/testgroupchat/alex".to_string(),
        my_chat,
        ResponsePortion::Any
    ));
    assert!(response_contains(
        &client,
        "/chat/testgroupchat/daniella".to_string(),
        my_chat,
        ResponsePortion::Any
    ));
    ...
    let response = client
        .post("/chat/testgroupchat/alex/1/delete")
...
fn admin_can_delete_any_chats() {
    ...
    for to_delete in chats_to_delete {
        ...
        for user in users {
            assert!(response_contains(
                &client,
                format!("/chat/testgroupchat/{user}"),
                to_delete,
                ResponsePortion::Any
            ));
        }
        ...
        let response = client
            .post(format!("/chat/testgroupchat/{admin}/0/delete"))
            .header(ContentType::Form)
            .dispatch();
        assert!(is_redirect(response.status()));
...
fn nonmembers_cant_delete_any_chats() {
    ...
    for deleter in deleters {
        ...
        let response = client
            .post(format!("/chat/testgroupchat/{deleter}/1/delete"))
            .header(ContentType::Form)
            .dispatch();
        assert!(is_redirect(response.status()));
