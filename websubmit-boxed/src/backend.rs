...
use mysql::Result; // is the point to not use mysql directly? this covers the Result return type from MySqlBackend
...
impl MySqlBackend { 
    pub fn new(
        ...
        prime: bool,
    ) -> Result<Self> { //bbox::db::QueryResult as Result wrapper (?) this isn't a query though
        let log = match log {
