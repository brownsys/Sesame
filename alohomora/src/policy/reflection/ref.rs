...
use crate::policy::{AnyPolicyTrait, AsLeaf, AsNoReflection, Policy, PolicyReflection, Reason};

pub type MutRefReflection<'r, 'a> = PolicyReflection<'a, &'r mut (dyn AnyPolicyTrait + 'static), &'r mut (dyn Policy + 'a)>;

impl<'r> AsLeaf for &'r mut (dyn AnyPolicyTrait + 'static) {
...
impl<'r, 'a: 'r> Policy for MutRefReflection<'r, 'a> {
    fn name(&self) -> String {
        match self {
