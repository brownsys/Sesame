...
use crate::policy::{AnyPolicyDyn, AnyPolicyTrait, AnyPolicyable, NoPolicy, Policy, PolicyAnd, PolicyDyn, PolicyDynInto, PolicyDynRelation, PolicyOr};
...
impl<P: AnyPolicyable + Serialize> AnyPolicySerialize for P {
    ...
    fn upcast_any_policy_mut(&mut self) -> &mut dyn AnyPolicyTrait { self }
    fn upcast_any_policy_box(self: Box<Self>) -> Box<dyn AnyPolicyTrait> { self }
    fn upcast_serialize(&self) -> &dyn Serialize { self }
    fn upcast_serialize_box(self: Box<Self>) -> Box<dyn Serialize> { self }
}
impl PolicyDyn for dyn AnyPolicySerialize {
    fn upcast_super(&self) -> &dyn AnyPolicyTrait { self.upcast_any_policy() }
    fn upcast_super_mut(&mut self) -> &mut dyn AnyPolicyTrait { self.upcast_any_policy_mut() }
    fn upcast_super_boxed(self: Box<Self>) -> Box<dyn AnyPolicyTrait> { self.upcast_any_policy_box()}
    fn upcast_ref(&self) -> &dyn Any { self.upcast_any_policy().upcast_any() }
    fn upcast_mut(&mut self) -> &mut dyn Any { self.upcast_any_policy_mut().upcast_any_mut() }
    ...
    fn no_policy() -> Box<Self> { Self::boxed_dyn(NoPolicy {}) }
...
impl<P: AnyPolicyable + Serialize + AnyPolicyMarker<dyn AnyPolicySerialize>> PolicyDynRelation<P> for dyn AnyPolicySerialize {
...
impl PolicyDynInto<dyn AnyPolicyTrait> for dyn AnyPolicySerialize {
    fn policy_dyn_into_ref(&self) -> &dyn AnyPolicyTrait { self.upcast_any_policy() }
    fn policy_dyn_into_boxed(self: Box<Self>) -> Box<dyn AnyPolicyTrait> { self.upcast_any_policy_box() }
}

mod __validation {
    pub fn example<P: super::AnyPolicySerialize>(_p: P) {}
    pub fn test_me(p: super::AnyPolicyDyn<dyn super::AnyPolicySerialize>) {
        example(p)
    }
}
// End of Macro.
