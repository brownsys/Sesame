...
declare_alohomora_lint!(
    ...
    "Sandboxable must always be auto-generated by the Sandboxable macro, not user-implemented",
...
fn contains_secret(cx: &LateContext<'_>, def_id: &DefId) -> bool {
    let secret = "Library implementation of Sandboxable. Do not copy this docstring!";
...
fn error_message(cx: &LateContext<'_>, &def_id: &DefId) {
    ...
    match map.span_if_local(def_id.clone()) {
        None => {
            panic!("Manual implementation of Sandboxable trait at {}. doc: {:?}",
        ...
        Some(span) => {
            span_lint_and_help (
                ...
                span,
                "\x1b[93mmanual implementation of Sandboxable trait\x1b[0m",
                None, "use `#[derive(Sandboxable)]` instead"
            );
        }
    }
}
...
fn check_alohomora_sandboxable_derived(cx: &LateContext<'_>) {
    let path: &[&str] = &vec!["alohomora_sandbox", "Sandboxable"];
