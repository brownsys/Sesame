...
declare_alohomora_lint!(
    ...
    pub ALOHOMORA_SANDBOX,
    Deny, // does not allow override
    "AlohomoraSandbox must always be auto-generated by AlohomoraSandbox, not user-implemented",
    check_alohomora_sandbox_derived
);
...
fn contains_secret(cx: &LateContext<'_>, def_id: &DefId) -> bool {
    let secret = "Library implementation of AlohomoraSandbox. Do not copy this docstring!";
...
fn error_message(cx: &LateContext<'_>, &def_id: &DefId) {
    ...
    match map.span_if_local(def_id.clone()) {
        None => {
            panic!("Manual implementation of AlohomoraSandbox trait at {}. doc: {:?}",
        ...
        Some(span) => {
            span_lint_and_help (
                ...
                ALOHOMORA_SANDBOX,
                span,
                "\x1b[93mmanual implementation of AlohomoraSandbox trait\x1b[0m",
                None, "use `#[AlohomoraSandbox()]` instead"
            );
        }
    }
}
...
// Lint implementation
fn check_alohomora_sandbox_derived(cx: &LateContext<'_>) {
    let path: &[&str] = &vec!["alohomora_sandbox", "AlohomoraSandbox"];
...
fn alohomora_sandbox_legal() {
    dylint_testing::ui_test_example(
        env!("CARGO_PKG_NAME"),
        "alohomora_sandbox_legal"
    );
}

#[test]
fn alohomora_sandbox_illegal() {
    dylint_testing::ui_test_example(
        env!("CARGO_PKG_NAME"),
        "alohomora_sandbox_illegal"
    );
}
