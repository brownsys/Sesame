...
impl<'a, 'r> PConResponder<'a, 'r, 'static> for PConRedirect {
    fn respond_to(self, request: PConRequest<'a, 'r>) -> PConResponseResult<'static> {
        match rocket::response::Responder::respond_to(self.redirect, request.get_request()) {
            Ok(response) => Ok(PConResponse::new(response)),
            Err(e) => Err(e),
        }
    }
}
...
mod tests {
    ...
    fn test_mixed_redirect() {
        ...
        let b1 = PCon::new(String::from("hello"), NoPolicy {});
        let b2 = PCon::new(10u32, NoPolicy {});
        let b3 = -20i32;
        let b4 = "my_str";

        let redirect =
            PConRedirect::to("/test/{}/more/{}/{}/less/{}", (&b1, &b2, &b3, &b4), context);
