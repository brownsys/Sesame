...
// This provides a generic representation for values, pcons, vectors, and structs mixing them.
pub enum SesameTypeEnum<T: SesameDyn + ?Sized = dyn Any, P: PolicyDyn + ?Sized = dyn AnyPolicyDyn> {
    PCon(PCon<Box<T>, AnyPolicy<P>>),
    Value(Box<T>),
...
impl<T: SesameDyn + ?Sized, P: PolicyDyn + ?Sized> SesameTypeEnum<T, P> {
    pub fn remove_pcon(self) -> (Self, Result<Option<AnyPolicy<P>>, ()>) {
        match self {
            SesameTypeEnum::Value(val) => (SesameTypeEnum::Value(val), Ok(None)),
            SesameTypeEnum::PCon(pcon) => {
                let (t, p) = pcon.consume();
