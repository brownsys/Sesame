...
#[derive(FromAttr)]
#[attribute(ident = sesame_out_type)]
struct SesameTypeArgs {
    name: Option<Ident>,
            ...
            TypeParamBound::Trait(TraitBound {
                ...
                path: Path {
                    ...
                    segments: Punctuated::from_iter([
                        PathSegment {
                            ident: Ident::new("sesame", Span::mixed_site()),
...
// $field_type: ::sesame::SesameType<__TDyn, __PDyn>
...
// Create a where clause bound on the form
// __TDyn: ::sesame::sesame_type_dyns::SesameDynRelation<$field_type>
...
fn derive_traits_for_output_type(attrs: &SesameTypeArgs) -> Option<Attribute> {
...
fn construct_out_fields(input: &ItemStruct, attrs: &SesameTypeArgs) -> Result<Fields, Error> {
    match &input.fields {
                ...
                .map(|field| {
                    ...
                    if !attrs.is_verbatim(&field.ident.as_ref().unwrap().to_string()) {
                        field.ty = Type::Verbatim(quote! {
                            <#ty as ::sesame::SesameTypeOut>::Out
                        });
                    }
                    field
                })
...
fn construct_out_type(
    input: &ItemStruct,
    attrs: &SesameTypeArgs,
    sesame_fields_types: &Vec<Type>,
    ...
    for field_type in sesame_fields_types {
        where_clause_out
            .as_mut()
            .unwrap()
            .predicates
...
pub fn derive_sesame_type_impl(input: DeriveInput) -> Result<TokenStream, Error> {
    ...
    let attrs = SesameTypeArgs::from_attributes(&input.attrs).unwrap();
    ...
    // Filter into those that are SesameTypes themselves, and those who are kept verbatim.
    let sesame_fields: Vec<_> = fields
        .iter()
    ...
    let sesame_fields_idents: Vec<_> = sesame_fields
        .iter()
        .map(|(ident, _, _)| ident.clone())
        .collect();
    let sesame_fields_strings: Vec<_> = sesame_fields
        .iter()
    ...
    let modified_input =
        add_dyn_generics_and_bounds(&input, &sesame_fields_types, &verbatim_fields_types);
    let (impl_generics_dyns, _, where_clause_dyns) = modified_input.generics.split_for_impl();

    // Construct the output struct.
    let out = construct_out_type(&input, &attrs, &sesame_fields_types)?;
    ...
    Ok(quote! {
        ...
        impl #impl_generics ::sesame::SesameTypeOut for #input_ident #ty_generics #where_clause_out {
            type Out = #out_ident #ty_generics;
        }

        #[automatically_derived]
        #[doc = "Library implementation of SesameType. Do not copy this docstring!"]
        impl #impl_generics_dyns ::sesame::SesameType<__TDyn, __PDyn>  for #input_ident #ty_generics #where_clause_dyns {
            fn to_enum(self) -> ::sesame::SesameTypeEnum<__TDyn, __PDyn>  {
                let mut map: ::std::collections::HashMap<::std::string::String, ::sesame::SesameTypeEnum> = ::std::collections::HashMap::new();
                ::sesame::SesameTypeEnum::Struct(::std::collections::HashMap::from([
                    #((String::from(#sesame_fields_strings), <#sesame_fields_types as ::sesame::SesameType<__TDyn, __PDyn> >::to_enum(self.#sesame_fields_idents)),)*
                    #((String::from(#verbatim_fields_strings), ::sesame::SesameTypeEnum::Value(
                        ::sesame::sesame_type_dyns::SesameDynRelation::boxed_dyn(self.#verbatim_fields_idents))
                    ),)*
                ]))
            }
            fn from_enum(e: ::sesame::SesameTypeEnum<__TDyn, __PDyn> ) -> Result<Self, ()> {
                match e {
                    ::sesame::SesameTypeEnum::Struct(mut hashmap) => {
                        Ok(Self {
                            #(#sesame_fields_idents: <#sesame_fields_types as ::sesame::SesameType<__TDyn, __PDyn> >::from_enum(hashmap.remove(#sesame_fields_strings).unwrap())?,)*
            ...
            fn out_from_enum(e: ::sesame::SesameTypeEnum<__TDyn, __PDyn> ) -> Result<Self::Out, ()> {
                match e {
                    ::sesame::SesameTypeEnum::Struct(mut hashmap) => {
                        Ok(Self::Out {
                            #(#sesame_fields_idents: <#sesame_fields_types as ::sesame::SesameType<__TDyn, __PDyn> >::out_from_enum(hashmap.remove(#sesame_fields_strings).unwrap())?,)*
                            #(#verbatim_fields_idents: hashmap.remove(#verbatim_fields_strings).unwrap().coerce()?,)*
                        })
                    },
                    _ => Err(()),
