...
#[proc_macro_derive(SesameType, attributes(sesame_out_type))]
pub fn derive_sesame_type(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    match sesame_type::derive_sesame_type_impl(input) {
...
#[proc_macro_attribute]
pub fn SesameSandbox(_args: TokenStream, input: TokenStream) -> TokenStream {
    let mut result = input.clone();
    let parsed = parse_macro_input!(input as ItemFn);
