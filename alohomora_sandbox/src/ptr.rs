...
/// `known_ptr` just has to be any global pointer that is in sandboxed memory, so we can determine its location.
pub fn swizzle_ptr<T>(ptr: &SandboxPointer<T>, known_ptr: usize) -> *mut T {
